...
- **rethink**: Invoke the **Harness Engineer** (Fresh `pro` subagent).
  - Pass the Engineer the *Failure Trajectories* (the 3 failed cycles).
  - The Engineer rewrites the SOP Task Packet (Constraints/Verification) to be strictly fool-proof, generating an *Executable Patch*.
  - Delegate the task back to the worker using the **Patched Harness**.
  - **HARD CAP**: Maximum **2 consecutive `rethink`** cycles. After 2 cycles, stop immediately and surface to user.
...
- **Harness-R1 Mechanics**: `references/harness-r1-mechanics.md`
